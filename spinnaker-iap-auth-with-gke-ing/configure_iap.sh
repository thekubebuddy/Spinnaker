...
bold "Setting-up spin CLI with the hostname name..."
[ ! -f ~/spin ] && ./install_spin.sh || bold "Spin already installed.."

cat >~/.spin/config <<EOL
gate:
  endpoint: https://$HOST_NAME/gate
...
hal config security api edit --override-base-url https://$HOST_NAME/gate
